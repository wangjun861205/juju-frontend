...
export const Create = () => {
	...
	return <div>
		...
		<OptionCreateComponent  setOptions={setOptions} />
...
export const Update = () => {
	...
	useEffect(() => {
		...
		.then(res => {
			...
			res.json().then(qst => setQuestion(qst)).catch(err => { message.error(err); nav(-1) })
		})
	}, [question_id]);
	...
	return question ? <div>
		...
		<OptionCreateComponent setOptions={setOptions}/>
		<OptionListComponent options={question!.options} setOptions={setOptions}></OptionListComponent>
	</div> :  <Spin />
}
